...
def dashboard(request):
    ...
    products = ProcurementList.objects.filter(
        expiry_date__range=[_past_5_days, _5_days_from_now]).all()
...
def product_list(request):
    ...
    if request.method == 'POST' and 'typeBtn' in request.POST:
        ...
        if form_type.is_valid():
            ...
            messages.error(
                request, 'Form Failed to Add Type. Check if type already exists. ')

    if request.method == 'POST' and 'pdfDownload' in request.POST:
        products = Products.objects.all().order_by('-ordered_date')
        dir_ = os.path.join(settings.BASE_DIR, 'media')
        html_string = render_to_string(
            'pdf/Pharmacy/product_list.html', {'queryset': products})
        html = HTML(string=html_string)
        html.write_pdf(target=f'{dir_}/tmp/product_list.pdf',
    ...
    if request.method == 'POST' and 'catBtn' in request.POST:
        ...
        if form_type.is_valid():
            ...
            messages.error(
                request, 'Failed to add Category. Check if category already exists. ')
    ...
    if request.method == 'POST' and 'productBtn' in request.POST:
        ...
        if form_type.is_valid():
            ...
            messages.error(
                request, 'Failed to add Product. Product must have a unique name and ID. Check if name or Id already exists. If it does, edit Product instead. ')
...
def add_customer_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            form.save()
            messages.success(
                request, f"Customer {name} was successfully saved. ")
            return redirect('super:add-customer')
        else:
            messages.error(
                request, "Data was not entered successfully. Check the errors below.")
...
def inventory(request):
    ...
    if request.method == 'POST' and 'PDFInventoryListButton' in request.POST:
        ...
        html = HTML(string=html_string)
        html.write_pdf(
            target=f'{dir_}/tmp/inventory_list.pdf', stylesheets=[CSS(string=boots)])
    ...
    if request.method == 'POST' and 'WholesalePrint' in request.POST:
        ...
        if querried_date in all_sales_dates:
            ...
            with fs.open('transaction_list.pdf') as pdf:
                response = HttpResponse(pdf, content_type='application/pdf')
                response[
                    'Content-Disposition'] = f'attachment; filename="wholesale-for-{querried_date}-list.pdf"'
                return response
        else:
            messages.error(
                request, 'Sales Data not available for the selected Date')
...
def procurement_list(request):
    ...
    if request.method == 'POST' and 'supplierReportPrint' in request.POST:
        ...
        if supply_date in product_dates_list:
            ...
            messages.error(
                request, 'Selected date does match existing Purchases. ')
...
def transaction_list(request):
    ...
    transaction_dates_group = [transact.date.strftime(
        "%Y-%m-%d") for transact in transactions]
    ...
    if request.method == 'POST' and 'TransactionPrint' in request.POST:
        ...
        if transaction_date in transaction_dates_group:
            ...
            messages.error(
                request, 'Selected date does match existing Transactions. ')
