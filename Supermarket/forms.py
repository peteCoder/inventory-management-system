...
class AddInvoiceForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        existing_invoice = Invoice.objects.filter(customer=customer)
        if existing_invoice:
            raise forms.ValidationError(
                {"customer": f"Invoice for customer: '{customer.name}' already exists."})
...
class addSupplierForm(forms.ModelForm):
    ...
    def clean_email(self):
        ...
        if not email or email == "":
            ...
            if "@" not in email:
                raise forms.ValidationError(
                    "Email must be valid. Try adding an '@' symbol to the email. ")
            for instance in Supplier.objects.all():
...
class SalesForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        current_customer = self.cleaned_data.get("customer")
        ...
        # Check if invoice exists upon entry and if the customer and invoice customer matches
        if bool(invoice):
            if invoice.customer.id != current_customer.id:
                raise forms.ValidationError(
                    {'invoice': 'The Invoice customer and the customer you entered \
                     do not match. Clear the invoice field if you are selling to a \
                     different customer or create a new invoice for that customer.'
                     })
        ...
        if int(quantity_sold) < 1:
            raise forms.ValidationError(
                {'quantity_sold': 'You cannot enter quantity less than 1'})

        if int(total_q_sold + quantity_sold) > int(product_available):
            raise forms.ValidationError(
                {'quantity_sold': f"You have already entered {total_q_sold} '{entered_product_name}' to sell. {quantity_sold} is a bit too much. {left_in_stock} left in stock."})
        ...
        if invoice:
            if invoice.payment_status:
                raise forms.ValidationError(
                    {'invoice': "The selected invoice has received payment. Create another invoice for the customer."})
...
class ProcurementForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if int(quantity_ordered) < 1:
            raise forms.ValidationError(
                {'quantity_ordered': 'You cannot enter quantity less than 1'})
